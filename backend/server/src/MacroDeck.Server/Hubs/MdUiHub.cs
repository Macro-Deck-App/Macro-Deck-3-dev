...
public class MdUiHub : Hub<IMdUiClient>
{
    ...
    public async Task<string> NavigateToView(string viewId, string? sessionId = null)
	{
		try
		{
			...
			_logger.Debug("Client {ConnectionId} navigating to view {ViewId} - Session: {SessionId} (provided: {ProvidedSessionId})", 
				Context.ConnectionId, viewId, actualSessionId, sessionId ?? "null");
			...
			lock (_connectionSessionsLock)
			{
				...
				_logger.Debug("Mapped session {SessionId} to connection {ConnectionId}. Total sessions: {Count}", 
					actualSessionId, Context.ConnectionId, _sessionToConnection.Count);
			}
    ...
    public async Task SendEvent(UiEventMessage eventMessage)
	{
		try
		{
			_logger.Debug("Received event {EventName} for view {ViewId} from session {SessionId}. Total sessions: {Count}",
			...
			lock (_connectionSessionsLock)
			{
				...
				{
					_logger.Warning("Session {SessionId} not found in session-to-connection mapping. Available sessions: {Sessions}", 
						eventMessage.SessionId, string.Join(", ", _sessionToConnection.Keys));
				}
				else if (mappedConnectionId != Context.ConnectionId)
				{
					_logger.Warning("Session {SessionId} is mapped to connection {MappedConnectionId}, but event came from {ConnectionId}", 
						eventMessage.SessionId, mappedConnectionId, Context.ConnectionId);
				}
				else
				{
					_logger.Debug("Session {SessionId} correctly mapped to connection {ConnectionId}", 
						eventMessage.SessionId, Context.ConnectionId);
				}
			}
